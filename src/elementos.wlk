...
object castillo {
	...
	method altura() = 20
	method recibirAtaque(unArma) { nivelDefensa = 0.max(nivelDefensa - unArma.potenciaDelArma()) }
...
object aurora {
	...
	method altura() = 1
	method recibirAtaque(unArma) { if(unArma.potenciaDelArma() >= 10 and estaViva) { estaViva = false } }
